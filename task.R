...
X=as.matrix(read.csv(file = "Downloads/X_16.csv",header= F))
colnames(X)=c("X1","X2","X3","X4")

Y=as.matrix(read.csv(file = "Downloads/y_16.csv",header = F))
colnames(Y)=c("Y")

Time= read.csv("Downloads/time_16.csv",header = F,skip = 1)
...
p2 <- plot(density(Y_training_data), col="blue", lwd=2,
           main="Distribution of Traning Data set")
